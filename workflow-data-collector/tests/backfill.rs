...
//!
//! These tests use a dedicated, per-run stream name on the test Redis
...
//!
//! Semantics under test (pinned in SPEC.md §3.5):
//! - the chosen range `[from, to]` is inclusive (XRANGE semantics);
//! - dedupe applies: entries at/below the **resume point** — `max(durable
//!   CHECKPOINT, highest id already written to JSONL)` — are skipped, so
//!   re-running a range never duplicates rows, including after a crash
//!   between appending and writing CHECKPOINT (BUG-WFDC-002);
//! - CHECKPOINT advances forward only, never backward;
...
//! - an inverted range (`--from` > `--to`) writes nothing and exits 0;
//! - deterministic output: same range in fresh data_dirs is byte-identical
//!   (BUG-WFDC-004).
...
/// Library path for one backfill run: store → startup repair → resume point
/// (`max(durable CHECKPOINT, highest id written to JSONL)`) → run. Mirrors
/// what `main.rs` does for the `backfill` subcommand.
fn run_backfill(
    cfg: &Config,
    from: &str,
    to: &str,
...
// ---------------------------------------------------------------------------
// 1. Range selection: only the chosen [from, to] window is written.
// ---------------------------------------------------------------------------
#[test]
fn backfill_writes_only_the_chosen_range() {
    ...
    let out = run_backfill(&cfg, from, to).expect("backfill ok");
...
// ---------------------------------------------------------------------------
// 2. Determinism (BUG-WFDC-004): same range → byte-identical raw rows and
//    same session_pk set.
// ---------------------------------------------------------------------------
#[test]
fn backfill_is_deterministic() {
    ...
    assert!(s1.iter().all(|r| r.state == State::Completed));
    let _ = std::fs::remove_dir_all(&d1);
    let _ = std::fs::remove_dir_all(&d2);
}

// ---------------------------------------------------------------------------
// 3. Dedupe/checkpoint (§3.1): entries at/below the resume point are skipped
//    and CHECKPOINT is only ever advanced forward.
// ---------------------------------------------------------------------------
#[test]
...
fn backfill_rerun_same_range_does_not_duplicate() {
    ...
    let out1 = run_backfill(&cfg, "1727500000000-0", "1727500000004-0").expect("first backfill");
    assert_eq!(out1.raw_lines, 5);
    assert_eq!(read_raw_lines(&dir).len(), 5);
    assert_eq!(checkpoint_of(&dir).as_deref(), Some("1727500000004-0"));

    // same range again → 5 lines, not 10 (stream_id <= resume point skipped)
    let out2 = run_backfill(&cfg, "1727500000000-0", "1727500000004-0").expect("second backfill");
    assert_eq!(out2.raw_lines, 0, "all entries <= resume point → skipped");
...
fn backfill_range_below_checkpoint_writes_nothing_and_keeps_checkpoint() {
    ...
    // written (dedupe), checkpoint never moved backward.
    let out = run_backfill(
        &cfg_for(&dir, &tr.stream),
        "1728000000002-0",
        "1728000000005-0",
    ...
    .expect("backfill ok");
    assert_eq!(out.raw_lines, 0, "all entries <= resume point are skipped");
...
fn backfill_partial_overlap_skips_below_and_writes_above_checkpoint() {
    ...
    // exactly once; checkpoint advances to the range end.
    let out = run_backfill(
        &cfg_for(&dir, &tr.stream),
        "1728100000005-0",
        "1728100000009-0",
...
// ---------------------------------------------------------------------------
// 3b. BUG-WFDC-002: the JSONL-watermark resume is ported to backfill startup.
//     A crash between appending a batch and writing CHECKPOINT leaves rows on
//     disk ahead of the durable checkpoint; the re-run must skip everything
//     at/below max(durable CHECKPOINT, highest id written to JSONL).
// ---------------------------------------------------------------------------
#[test]
fn crash_window_regressed_checkpoint_rerun_does_not_duplicate() {
    let mut tr = TestRedis::new();
    // Seed 1000-0..1006-0 (7 events) — the DED repro.
    for i in 0..7u64 {
        let id = format!("{}-0", 1000 + i);
        tr.xadd(&id, &start_fields("dev", "s1"));
    }
    let dir = temp_data_dir("ded");
    let cfg = cfg_for(&dir, &tr.stream);

    // Run 1: full backfill → 7 unique rows, checkpoint = 1006-0.
    let out1 = run_backfill(&cfg, "0", "+").expect("first backfill");
    assert_eq!(out1.raw_lines, 7);
    assert_eq!(read_raw_lines(&dir).len(), 7);
    assert_eq!(checkpoint_of(&dir).as_deref(), Some("1006-0"));

    // Simulate the crash window: the durable CHECKPOINT is *behind* the
    // highest id written to JSONL.
    write_checkpoint(&dir, "1002-0");

    // Re-run the same backfill → must be 7 unique rows, 0 duplicates.
    let out2 = run_backfill(&cfg, "0", "+").expect("re-run backfill");
    assert_eq!(
        out2.raw_lines, 0,
        "nothing re-written (all at/below resume point)"
    );
    let all = read_raw_lines(&dir);
    assert_eq!(all.len(), 7, "7 unique rows");
    let ids: std::collections::BTreeSet<&str> = all
        .iter()
        .map(|r| r["stream_id"].as_str().unwrap())
        .collect();
    assert_eq!(ids.len(), 7, "0 duplicates");
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn backfill_pairs_sessions_and_marks_orphan_finish() {
    ...
    let states: Vec<State> = sessions.iter().map(|r| r.state).collect();
    assert!(
        states.contains(&State::Completed),
        "y pair completed: {states:?}"
    );
    assert!(
        states.contains(&State::OrphanFinish),
        "finish without in-range start is orphan: {states:?}"
    );
    assert!(
        states.contains(&State::Open),
...
fn backfill_finish_pairs_with_start_flushed_in_earlier_run() {
    ...
    let out1 = run_backfill(&cfg, "1732000000000-0", "1732000000000-0").expect("run1");
    assert_eq!(out1.raw_lines, 1);
    let s1 = read_session_rows(&dir);
    assert_eq!(s1.len(), 1);
    assert_eq!(s1[0].state, State::Open);

    // Run 2: only the finish. The start is <= resume point → dedupe-skipped
    // from raw, but the finish must still pair with the open row rebuilt from
    // disk.
    let out2 = run_backfill(&cfg, "1732000000001-0", "1732000000001-0").expect("run2");
    assert_eq!(out2.raw_lines, 1);
    let s2 = read_session_rows(&dir);
    assert_eq!(s2.len(), 1, "still one session row — completed, not orphan");
    assert_eq!(s2[0].state, State::Completed);
    assert_eq!(
...
fn follow_preserves_session_rows_from_earlier_backfill() {
    ...
    let out = run_backfill(&cfg, "1735000000000-0", "1735000000000-0").expect("backfill start");
    ...
    assert_eq!(rows2[0].state, State::Completed);
    assert_eq!(
...
fn backfill_inverted_range_is_clean_noop_and_exits_zero() {
    ...
    let out =
        run_backfill(&cfg_for(&dir, &tr.stream), from, to).expect("inverted range is not an error");
...
//    holder makes backfill exit 3; a stale lock is taken over.
// ---------------------------------------------------------------------------
#[test]
